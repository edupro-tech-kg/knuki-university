...
const DocumentTable = ({
  ...
  const renderActionButton = (item, index) => {
    ...
    const buttonLabel = typeof buttonText === "function" ? buttonText(item, index) : buttonText;
    ...
    if (item.list && Array.isArray(item.list)) {
      return (
        <div>
          ...
          {item.footer && <p className={`mt-2 italic ${textSize} text-gray-600`}>{item.footer}</p>}
        </div>
      );
    }
  ...
  const renderTableRow = (item, index, isGroupHeader = false) => {
    if (isGroupHeader) {
      ...
      return (
        <tr key={`group-${item[groupIdKey] || index}`}>
          <td colSpan={colSpan} className="py-3 sm:py-4 border-b border-t border-black bg-gray-50">
    ...
    return (
      ...
      >
        ...
        <td
          className={`${textColumnClass} ${shouldShowAction ? "border-r border-black" : ""} bg-white`}
        >
        ...
        {shouldShowAction && (
          <td className={`${actionColumnWidth} px-4 py-3 bg-white`}>
            <div className="flex justify-end">{renderActionButton(item, index)}</div>
          </td>
        )}
      </tr>
    );
  ...
  const renderMobileCard = (item, index, isGroupHeader = false) => {
    if (isGroupHeader) {
      ...
      return (
        ...
        >
          <h3 className="text-base sm:text-lg font-semibold text-center">{getItemText(item)}</h3>
        </div>
      );
    }
    ...
    return (
      ...
      >
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1">{renderItemContent(item, index)}</div>
        </div>

        {shouldShowAction && (
          <div className="flex justify-end pt-2">{renderActionButton(item, index)}</div>
        )}
      </div>
    );
  };
            ...
            {data.map((group, groupIndex) => {
              ...
              return (
                <React.Fragment key={`group-${group[groupIdKey] || groupIndex}`}>
                  ...
                  {groupItems.map((item, itemIndex) => renderTableRow(item, itemIndex))}
                </React.Fragment>
              );
            })}
          </tbody>
    ...
    return data.map((group, groupIndex) => {
      ...
      return (
        <React.Fragment key={`group-${group[groupIdKey] || groupIndex}`}>
          ...
          {groupItems.map((item, itemIndex) => renderMobileCard(item, itemIndex))}
        </React.Fragment>
      );
    });
  };
  ...
  return (
    <div className={`${className}`}>
      ...
      {renderExtraContent && <div className="mt-6">{renderExtraContent()}</div>}
    </div>
  );
};
